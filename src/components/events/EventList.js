...
export const EventList = ({}) => {
  ...
  const fetchSortedCall = async () => {
    const fetchData = await fetch(
      `http://localhost:8088/events?_embed=patronSavedEvents&_expand=eventType&_sort=startDate&_order=desc`
    );
  ...
  const sortFavorites = () => {
    if (events.length > 0 && events[0].name !== undefined) {
      ...
      if (sortFavorited) {
        copy.sort(
          (a, b) => b.patronSavedEvents.length - a.patronSavedEvents.length
        );
        setFilteredEvents(copy);
      } else {
        copy.sort(
          (a, b) => a.patronSavedEvents.length - b.patronSavedEvents.length
        );
        setFilteredEvents(copy);
      }
    }
      ...
      {events.length > 0 && events[0].name !== undefined ? (
        <article className="events">
          <div className="filterBar">
            <img
              src={require("./event-images/calendar.png")}
              alt=""
              className="calendar"
              onClick={() => {
